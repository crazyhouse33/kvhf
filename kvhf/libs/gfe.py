...
def search_files(self, paths):
    if type(paths) == str:#manage mono path
        return paths, (self.tree/os.path.relpath(paths,start=self.repo.working_dir)).data_stream 
    #git python force to use path relative to repo
    return [(path, (self.tree/os.path.relpath(path,start=self.repo.working_dir)).data_stream) for path in paths]

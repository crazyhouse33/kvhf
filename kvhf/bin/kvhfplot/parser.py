...
parser = argparse.ArgumentParser(
    "kvhfplot is an utility to generate graphs from kvhf files.")
...
parser.add_argument(
    ...
    metavar='FILES',
    nargs='+',
    help="The files to plot, if more that one given operate on the concatenation of them")
...
parser.add_argument(
    ...
    '--comparison-chart',
    action='store_true',
    help='Draw a comparison chart unstead of curves if the selection match one label only')
...
parser.add_argument('-r', '--labels-rotation', default=0, type=float,
                    help='Rotation of labels in degrees. Usefull when you get some overlapping')
# test only option
parser.add_argument(
    '--dont-block',
    action='store_false',
